...
def spin_roulette():
    if listbox.size() > 0:
        ...
        for _ in range(spins):
            ...
            result_label.config(text=f"Girando... {selected_item}")
            root.update()  # Update the GUI to show the spinning effect
            time.sleep(delay)

        selected_index = random.randint(0, listbox.size() - 1)
        selected_item = listbox.get(selected_index)
        result_label.config(text=f"Resultado: {selected_item}")
    else:
        result_label.config(text="Añade elementos a la ruleta primero!")
...
label = tk.Label(frame, text="Ingresa un elemento:")
...
saved_label = tk.Label(root, text="Elementos añadidos")
